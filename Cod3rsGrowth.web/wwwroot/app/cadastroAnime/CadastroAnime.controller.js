sap.ui.define([
	...
	const CAMINHO_PARA_API_ADICIONAR_ANIMEGENERO = "/api/anime/adicionar/animegenero";
    ...
    return ControleBase.extend("ui5.anime.app.cadastroAnime.CadastroAnime", {
		...
		_postAnime: async function(url, anime){
			this._exibirEspera(async () => {
				...
				if(response.ok){
					this._sucessoNoPost();
					var idGeneros = this._preencherAListaDeGenerosSelecionados();
					this._postAnimeGenero(CAMINHO_PARA_API_ADICIONAR_ANIMEGENERO,idGeneros);
				}
				else{
		...
		_postAnimeGenero: async function(url,idGeneros){
			this._exibirEspera(async () => {
				const response = await fetch (url,{
					method: "POST",
					headers: {
